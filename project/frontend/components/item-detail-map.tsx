...
export default function ItemDetailMap({ item }: ItemDetailMapProps) {
    ...
    return (
        <div className={`relative w-full h-full rounded-xl overflow-hidden shadow-lg border border-slate-200 transition-all duration-500 ${isFullscreen ? "fixed inset-0 z-50 rounded-none" : "h-full"}`}>
            <Map
                ref={mapRef}
